...
requirements_file = pkg_resources.resource_filename('pyGnirsIFU', 'requirements.txt')
...
for requirement in install_requires:
    ...
    try:
        ...
        if pkg_resources.parse_version(pv) < pkg_resources.parse_version(version):
            raise ImportError('Your version of {0} is incompatible with pyGnirsIFU.  '.format(pkg)
